...
pub struct PlainRecordJsonRiferimentoNISECI {
...
impl RecordRiferimentoNISECI for PlainRecordJsonRiferimentoNISECI {
...
impl fmt::Display for PlainRecordJsonRiferimentoNISECI {
...
#[derive(Debug, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct PlainRecordJsonCampionamentoNISECI {
    pub data: String,
    pub stazione: String,
...
impl RecordCampionamentoNISECI for PlainRecordJsonCampionamentoNISECI {
    fn data(&self) -> String {
        self.data.clone()
    }
...
impl fmt::Display for PlainRecordJsonCampionamentoNISECI {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let string_representation = format!(
...
#[derive(Debug, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct PlainRecordJsonAnagraficaNISECI {
...
impl RecordAnagraficaNISECI for PlainRecordJsonAnagraficaNISECI {
...
impl fmt::Display for PlainRecordJsonAnagraficaNISECI {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let string_representation = format!(
    ...
    T: RecordRiferimentoNISECI,
{
    ...
    check_riferimento_niseci_reader(file).map_err(JsonPathCheckError::Json)
}
...
{
    let file = File::open(path)?;
    check_campionamento_niseci_reader(file).map_err(JsonPathCheckError::Json)
}
...
{
    let file = File::open(path)?;
    check_anagrafica_niseci_reader(file).map_err(JsonPathCheckError::Json)
}
